...
(function($){
    ...
    function changeLang( lang ){
        $('span[data-lang]').hide();
        _.each(o.langs, function(key, val){
          if (val != lang) {
            $('span[data-lang="'+ val +'"]').hide();
          }
        });
    ...
    function createLanguageSelect(){
        var template = $('<div style="z-index: 2;" class="language-select"><ul></ul></div>');

        $.each(o.langs, function(i, val){
            template.find('ul').append($('<li data-lang="' + i + '"><a href="#' + i + '" class=' + i + '>' + val + '</a></li>'));
        });
